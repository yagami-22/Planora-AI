...
export default function Home() {
  ...
  async function generateTimetable() {
    ...
    const scored = subjects.map((s) => {
      ...
      const weaknessScore = 100 - s.progress;
      const priorityScore =
    ...
    const plan = days.map((day, i) => {
      ...
      let task = "Concept Study";

      if (s.progress < 35) task = "Weak Topic Recovery + Basics";
      else if (s.daysLeft <= 3) task = "Final Revision + Mock Test";
      else if (s.daysLeft <= 7) task = "Revision + PYQs";
      else if (s.priority === "High") task = "High Priority Practice";
      else if (s.progress >= 70) task = "Quick Revision + Test";

      return {
        day,
        subject: s.subject_name,
        time: `${s.daily_hours} hour(s)`,
        task,
        priority: s.priority,
        daysLeft: s.daysLeft,
        aiScore: s.aiScore,
      ...
      <section className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
            ...
            <input className="w-full p-3 mb-3 bg-black border border-zinc-700 rounded-xl" placeholder="Subject name" value={subjectName} onChange={(e) => setSubjectName(e.target.value)} />
            <input type="date" className="w-full p-3 mb-3 bg-black border border-zinc-700 rounded-xl" value={examDate} onChange={(e) => setExamDate(e.target.value)} />
            <input type="number" className="w-full p-3 mb-3 bg-black border border-zinc-700 rounded-xl" placeholder="Daily free hours" value={dailyHours} onChange={(e) => setDailyHours(e.target.value)} />
            <input type="number" className="w-full p-3 mb-3 bg-black border border-zinc-700 rounded-xl" placeholder="Progress %" value={progress} onChange={(e) => setProgress(e.target.value)} />

            <select className="w-full p-3 mb-3 bg-black border border-zinc-700 rounded-xl" value={priority} onChange={(e) => setPriority(e.target.value)}>
              <option>Low</option>
              <option>Medium</option>
              <option>High</option>
            </select>

            <button onClick={addSubject} className="w-full bg-purple-600 p-3 rounded-xl font-semibold mb-3">
              {loading ? "Saving..." : "Add Subject"}
            </button>

            <button onClick={generateTimetable} className="w-full bg-white text-black p-3 rounded-xl font-semibold mb-3">
              Generate & Save AI Timetable
            </button>

            <button onClick={exportPDF} className="w-full bg-green-500 text-black p-3 rounded-xl font-semibold mb-3">
              Export Timetable as PDF
            </button>

            <button onClick={clearAllData} className="w-full bg-red-600 p-3 rounded-xl font-semibold">
              Clear All Data
            </button>
          </div>
          ...
          <div className="lg:col-span-2 bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="bg-black p-4 rounded-xl">
                ...
                <h3 className="text-3xl font-bold text-red-400">{weakSubjects.length}</h3>
              </div>

              <div className="bg-black p-4 rounded-xl">
                <p className="text-gray-400">Average Progress</p>
                <h3 className="text-3xl font-bold text-green-400">{averageProgress}%</h3>
              </div>
            </div>
            ...
            {subjects.length === 0 ? (
              ...
              <div className="space-y-3">
                {subjects.map((s) => (
                  <div key={s.id} className="bg-black p-4 rounded-xl flex justify-between items-center">
                    <div>
                      <h4 className="font-semibold">{s.subject_name}</h4>
                      <p className="text-sm text-gray-400">Exam Date: {s.exam_date}</p>
                    </div>

                    <div className="flex gap-4 items-center">
                      <span>{s.progress}%</span>
                      <span className="bg-purple-600 px-3 py-1 rounded-full text-sm">{s.priority}</span>
                      <button onClick={() => deleteSubject(s.id)} className="bg-red-600 px-3 py-1 rounded-lg text-sm">
                        Delete
                      </button>
                    </div>
                  </div>
        ...
        <div id="pdf-section" className="mt-6 bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
        ...
        <div className="mt-6 bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
          <h2 className="text-2xl font-semibold mb-5 text-red-400">Weak Subject Analysis</h2>

          {weakSubjects.length === 0 ? (
            <p className="text-gray-500">No weak subjects detected.</p>
          ) : (
            weakSubjects.map((s) => (
              <div key={s.id} className="bg-black border border-red-900 p-4 rounded-xl mb-3">
                <h3 className="font-bold text-red-400">{s.subject_name}</h3>
                <p className="text-gray-400">
                  Needs extra focus because priority is {s.priority} and progress is {s.progress}%.
                </p>
              </div>
            ))
          )}
